 ...
 * This function can handle nested JSON objects and arrays.
...
/**
 * Extracts keys and their values from a JSON array object based on the keys defined in a YANG list.
 * This function is used to map JSON data to YANG schema for list keys.
...
bool state_container_is_found = false;
/**
 ...
 * @param [in] search_state_only: Flag to indicate whether to process only state data.
...
void process_node(const struct lysc_node *s_node, json_object *json_array_obj,
                  bool search_state_only, struct lyd_node **parent_data_node)
{
    ...
    if (*parent_data_node == NULL) { // Top-level node
        ...
        case LYS_LEAFLIST: {
            ...
            /*
            if search state container is enabled, and the container is not found,
            it means that the leaf is in another container, don't process such leaf.*/
            if (search_state_only == true && state_container_is_found == false)
                break;
    ...
    {
        if (s_child->nodetype == LYS_CONTAINER && search_state_only == true &&
            state_container_is_found == false) {
            if (strcmp(s_child->name, "state") == 0) {
                state_container_is_found = true;
            }
        }
        process_node(s_child, json_array_obj, search_state_only, &new_data_node);
    }
}
 ...
 * @param [in] search_state_only: Flag to indicate whether to map only the data contained in schema "state" container only.
...
void jarray2lyd(json_object *json_array_obj, const struct lysc_node *top_node,
                bool search_state_only, struct lyd_node **data_tree)
{
    state_container_is_found = false;
    ...
    LY_LIST_FOR(top_node, node)
    {
        ...
        process_node(node, json_array_obj, search_state_only, data_tree);
    }
}
                           ...
                           struct lyd_node **parent)
{
    ...
    for (size_t i = 0; i < n_arrays; i++) {
        ...
        jarray2lyd(array_obj, module->compiled->data, true, &data_tree);
